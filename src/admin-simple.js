...
module.exports = function(app, getPool, initializeDatabase) {
  ...
  app.post('/admin/update-schema', async (req, res) => {
    try {
      ...
      // CRITICAL PERFORMANCE INDEXES for map-based queries (simplified for now)
      await pool.query('CREATE INDEX IF NOT EXISTS idx_events_map_query ON events (latitude, longitude, date, is_public);');
      await pool.query('CREATE INDEX IF NOT EXISTS idx_events_venue_public ON events (venue_id, date, is_public);');
